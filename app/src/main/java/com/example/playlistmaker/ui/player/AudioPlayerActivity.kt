...
import com.example.playlistmaker.presentation.adapters.dpToPx
import com.example.playlistmaker.presentation.controllers.AudioPlayerController
...
class AudioPlayerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        backButton.setNavigationOnClickListener {
            if (audioPlayerController.playerState == AudioPlayerController.STATE_PLAYING) {
                audioPlayerController.pausePlayer()
            }
            finish()
        }

        audioPlayerController = AudioPlayerController(track).apply {
            onProgressUpdate = { progress ->
                runOnUiThread { timePlayTrack.text = progress }
            }
            onCompletion = {
                runOnUiThread {
                    playTrackButton.setImageResource(R.drawable.ic_play)
                    timePlayTrack.text = getString(R.string.time_start)
                }
            }
        }
        ...
        playTrackButton.setOnClickListener {
            when (audioPlayerController.playerState) {
                AudioPlayerController.STATE_PLAYING -> {
                    audioPlayerController.pausePlayer()
                    playTrackButton.setImageResource(R.drawable.ic_play)
                }

                AudioPlayerController.STATE_PREPARED, AudioPlayerController.STATE_PAUSED -> {
